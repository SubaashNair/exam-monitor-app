...
// TestRoomNameToPort_ContractWithClient pins the inputs/outputs that both
// server and client MUST agree on. If you change the algorithm here, change
// it identically in client/util.go AND update this table in BOTH places.
func TestRoomNameToPort_ContractWithClient(t *testing.T) {
	cases := []struct {
		name string
		in   string
		want int
